...
int error = 0;
atomic_long waiting = ATOMIC_VAR_INIT(0);
long wait_count = 0;
...
void *dothread(void *arg){
    ...
    while(running_flag=1){
		...
		rounds++;
		//error--;
		wait_count += atomic_fetch_sub_explicit(&waiting, 1, memory_order_release);
...
/*
	for(int i=0; i<Num_core; i++){
	 	fprintf(fp, "thread %d enter CS: %d\n", i, thread_cs_counter[i]);
	 }
*/
...
int print_all(){
	...
	wait_count-=rounds;
	printf("waiting: %.2f", wait_count/rounds);
